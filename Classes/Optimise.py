...
class hyperparameter_optimiser:
    ...
    def __init__(self, model_name: str):
        ...
        self.models_path, self.parameters_path, self.data_path, self.search_space_path = Configs['directories']['best_model'], Configs['directories']['parameters'], Configs['directories']['datasets'], Configs['directories']['search_space'] 
    ...
    def store_results(self):
        ...
        if not os.path.exists('./Models/Opt_Results'):
            os.makedirs('./Models/Opt_Results')
        results_df.to_csv(f'./Models/Opt_Results/{self.model_name}_opt_results.csv', index = False)
...
# Function that optimises the VAE's hyperparameters
    # def build_optimised_vae(self):   
    ...
    #             epsilon = K.random_normal(shape=(K.shape(z_mean)[0], self.layers_dims[-1]), mean=0., stddev=1.)
    ...
    #     self.best_DL_model = self.tuner.get_best_hyperparameters(num_trials = 1)
    #     # Returns the best model's hyperparameters
    #     # return self.best_DL_model