...
default_tests_path = './Results/Default_Tests'
time_tests_path = './Results/Times'
dr_time_tests_path = './Results/Times/DR'
...
dr_techniques = ['AE', 'ICA', 'NMF', 'PCA', 'RF', 'RFE', 'SVD']
...
""" the table will have 5 different columns
    DR | Performance Increase (p/ comp.) | Time increase (p/ comp.) | Total performance increase | Total time increase

    The performance increase per component will be calculated by the formula: 
    (MCC[All] - MCC[2]) / (max_components - min_components)
    The time is exactly the same thing, per component

    For the total performance and time increases, the formula will be:
    (MCC[All] - MCC[2]) / MCC[2] and (Time[All] - Time[2]) / Time[2]
"""
    
# dr_techniques = ['PCA', 'ICA', 'NMF', 'RF', 'RFE', 'SVD', 'AE']
dr_techniques = ['PCA', 'ICA', 'NMF', 'RF', 'RFE', 'SVD', 'AE']
